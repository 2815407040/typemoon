  ...
  <div class="contribution-container">
    <el-card>
      ...
      <div v-if="loading" class="loading-state">
        <el-loading :visible="loading" text="加载中..." background="rgba(255, 255, 255, 0.8)" />
      </div>
      ...
      <div class="action-buttons" v-if="currentContribution && !loading">
        ...
        <el-button type="warning" @click="handleReview(2)">待定</el-button>
        <el-button type="danger" @click="handleReview(3)">驳回</el-button>
      </div>
    </el-card>
  </div>
</template>
...
const handleReview = async (status: number) => {
  ...
  try {
    // 发送PATCH请求更新审核状态
    await axios.patch(
        `http://localhost:3001/api/contributions/${currentContribution.value.id}`,
        { check: status }
    )
    ElMessage.success('审核操作成功')

    // 更新本地列表（移除已审核项）
    const index = contributions.value.findIndex(item => item.id === currentContribution.value!.id)
    if (index > -1) {
